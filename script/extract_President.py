...
df1: pd.DataFrame = pd.read_csv('tmp\President_tenure.csv')#读取职业生涯信息
series1: pd.Series = df1.set_index(['person_id', 'start_tenure', 'end_tenure']).index
person_group1 = df1.groupby('person_id')
promote_time = person_group1.apply(lambda s: maskfuc(s, 'president'))
work_before = person_group1.apply(lambda s: work(s, 'president'))
df2: pd.DataFrame = pd.read_csv('tmp\President_rank.csv')
